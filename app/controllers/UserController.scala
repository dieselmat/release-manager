...
class UserController @Inject()(repo: UserRepository,
  ...
  def addUser = Action.async { implicit request =>
    ...
    userForm.bindFromRequest.fold(
      ...
      errorForm => {
        Future.successful(Ok(views.html.user(errorForm)))
      },
      // There were no errors in the from, so create the user.
      user => {
        repo.create(user.name, user.email, true).map { _ =>
 ...
 */
case class CreateUserForm(name: String, email: String, isActive: Boolean)
